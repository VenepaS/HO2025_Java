...
public class PostcodeLookupSteps {
    ...
    public void iLookUpThePostcode(String postcode) {
        when().get(createEnvironmentVariables().getProperty("postcode.api.postcodes")+ postcode);
    }
    ...
    @And("the admin_district code should be {string}")
    public void theAdmin_districtCodeShouldBe(String expected) {
        then().body("result.codes.admin_district", equalTo(expected));
    }
